...
def three_layer_convnet(x, params):
    ...
    ######################CONV_LAYER1###################################
    l = 0
    vari1 = tf.constant([[0, 0], [2, 2], [2, 2], [0, 0]])
    h1 = tf.nn.conv2d(tf.pad(x, vari1, 'CONSTANT'), conv_w1, [1, 1, 1, 1], 'VALID') + conv_b1
    a1 = tf.nn.relu(h1)
    l = l+1
    ######################CONV_LAYER2###################################
    pad2 = tf.constant([[0, 0], [1, 1], [1, 1], [0, 0]])
    h2 = tf.nn.conv2d(tf.pad(a1, pad2, 'CONSTANT'), conv_w2, [1, 1, 1, 1], 'VALID') + conv_b2
    a2 = tf.nn.relu(h2)
    l = l+2
...
class ThreeLayerConvNet(tf.keras.Model):
    ...
    def call(self, x, training=None):
        ...
        vari = tf.constant([[0, 0], [2, 2], [2, 2], [0, 0]])
        a1 = self.conv1(tf.pad(x, vari, 'CONSTANT'))
        pad2 = tf.constant([[0, 0], [1, 1], [1, 1], [0, 0]])
        a2 = self.conv2(tf.pad(a1, pad2, 'CONSTANT'))
        a2_flat = tf.layers.flatten(a2)
        scores = self.fc(a2_flat)
...
# def myConvNet_functional(inputs, num_classes):
#     initializer = tf.variance_scaling_initializer(scale=2.0)
#     regularizer = tf.contrib.layers.l2_regularizer(scale=0.1)
    
#     inputs = tf.layers.conv2d(inputs=inputs, filters=32, kernel_size=(3, 3), strides=(1, 1),
#                               padding='same', data_format='channels_last', activation=tf.nn.relu,
#                               kernel_initializer=initializer)
#     inputs = tf.layers.batch_normalization(inputs)
#     inputs = tf.layers.conv2d(inputs=inputs, filters=64, kernel_size=(3, 3), strides=(1, 1),
#                               padding='same', data_format='channels_last', activation=tf.nn.relu,
#                               kernel_initializer=initializer)
#     inputs = tf.layers.batch_normalization(inputs)
#     inputs = tf.layers.max_pooling2d(inputs=inputs, pool_size=(2, 2), strides=(2, 2),
...
def my_model(X, is_training):
    ...
    # Output FC 
    scores = tf.layers.dense(inputs=bn3act, units=10, activation=None)
...
def model_init_fn(inputs, is_training):
    ...
    model = my_model(inputs, is_training)
...
# For achieving an accuracy of around 70%, I used the functional API of ```ts.layers```. Also, in my modified CNN model, I used ```batch normalization``` and ```max pooling``` in addition to the convolution and fully connected layers.
# 
# Sequentially, the layers used in my ConvNet include:
# 1. Convolution Layer with a 3 X 3 X 32 filter and a stride of 1
# 2. ReLU Activation
# 3. Batch Normalization
# 4. Convolution Layer with a 3 X 3 X 64 filter and a stride of 1
# 5. ReLU Activation
# 6. Batch Normalization
# 7. Max Pooling with a pool size of 2 and stride of 2
# 8. Flattening the result
# 9. Fully-Connected Layer of 1024 units without any activation function
# 10. Batch Normalization
# 11. Fully-Connected Layer of 10 units which represents the total number of classes in the dataset and which finally gives the class scores
# 
# Also, instead of the ```MomentumOptimizer```, I used the ```AdamOptimizer``` with a learning rate of 0.001.
# 
# Best accuracy obtained = 71.30%
